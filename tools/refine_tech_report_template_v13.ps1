param(
  [string]$InputPath = '',
  [string]$OutputPath = 'C:\Users\jan.jedlicka\Documents\PROJEKT_AI\UDIMO\sablona_technicke_zpravy_v1.2_navrh.docx'
)
...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('tech-template-v12-' + [guid]::NewGuid().ToString('N'))
$sourceZip = Join-Path ([IO.Path]::GetTempPath()) ('tech-template-v12-src-' + [guid]::NewGuid().ToString('N') + '.zip')
$outZip = Join-Path ([IO.Path]::GetTempPath()) ('tech-template-v12-out-' + [guid]::NewGuid().ToString('N') + '.zip')
...
try {
  ...
  # Main publicity row: EU first, MZP second, both with the same height.
  Update-AnchorByRid -Document $document -Rid 'rId8' -HOffset '700000'  -VOffset '5930000' -Cx '2642361' -Cy '387931'
  Update-AnchorByRid -Document $document -Rid 'rId9' -HOffset '3790000' -VOffset '5930000' -Cx '1500000' -Cy '387931'

  # Visual separator below the main funding logos.
  Update-AnchorByDocPrId -Document $document -Id '1214179664' -HOffset '940000' -VOffset '6640000' -Cx '3920000'

  # Cities placed as a smaller, clearly secondary group.
  Update-AnchorByRid -Document $document -Rid 'rId10' -HOffset '1540000' -VOffset '6920000' -Cx '766171'  -Cy '430000'
  Update-AnchorByRid -Document $document -Rid 'rId11' -HOffset '2660000' -VOffset '6920000' -Cx '1290618' -Cy '430000'

  # Add a centered "za podpory" label.
  ...
  Prepare-AnchorClone -Document $document -Anchor $supportLabelAnchor -Name 'Support label' -HOffset '1730000' -VOffset '7700000' -Cx '2400000' -Cy '260000'
  Set-TextBoxLabel -Anchor $supportLabelAnchor -Text 'za podpory' -HexColor '233848' -HalfPointSize '24'

  # Support logos: UDIMO + Dopravni podnik in a dedicated row.
  $supportLogoSpecs = @(
    @{
      Rid = 'rId20'
